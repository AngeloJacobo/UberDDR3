...
module ddr3_controller #(
    ...
    (
        ...
        input wire[WB2_ADDR_BITS - 1:0] i_wb2_addr, //burst-addressable {row,bank,col} 
        input wire[WB2_DATA_BITS - 1:0] i_wb2_data, //write data, for a 4:1 controller data width is 8 times the number of pins on the device
        // Wishbone 2 (Controller) outputs
        output reg o_wb2_stall, //1 = busy, cannot accept requests
        output reg o_wb2_ack, //1 = read/write request has completed
        output reg[WB2_DATA_BITS - 1:0] o_wb2_data, //read data, for a 4:1 controller data width is 8 times the number of pins on the device
        //
        // PHY interface 
    ...
    reg[$clog2(LANES)-1:0] wb2_phy_odelay_data_ld;
    reg[$clog2(LANES)-1:0] wb2_phy_odelay_dqs_ld;
    reg[$clog2(LANES)-1:0] wb2_phy_idelay_data_ld;
    reg[$clog2(LANES)-1:0] wb2_phy_idelay_dqs_ld;
    ...
    always @(posedge i_controller_clk, negedge i_rst_n) begin
        if(!i_rst_n) begin
            ...
            if(wb2_update) begin
                odelay_data_cntvaluein[wb2_write_lane] <= wb2_phy_odelay_data_cntvaluein;
                odelay_dqs_cntvaluein[wb2_write_lane] <= wb2_phy_odelay_dqs_cntvaluein;
                idelay_data_cntvaluein[wb2_write_lane] <= wb2_phy_idelay_data_cntvaluein;
                idelay_dqs_cntvaluein[wb2_write_lane] <= wb2_phy_idelay_dqs_cntvaluein;
  ...
  BITSLIP_DQS_TRAIN_2: if(train_delay == 0) begin //train again the ISERDES to capture the DQ correctly
   ...
   always @(posedge i_controller_clk, negedge i_rst_n) begin
       if(!i_rst_n) begin
           ...
           wb2_phy_idelay_dqs_ld <= 0;
           wb2_update <= 0;
           o_wb2_ack <= wb2_stb; //always ack right after request
           o_wb2_stall <= 0; //no need to stall
           ...
           if(wb2_stb) begin
                case(wb2_addr) 
                    ...
                    0: if(wb2_we) begin 
                            ...
                            wb2_phy_odelay_data_ld <= wb2_data[5 +: $clog2(LANES)]; //save lane to be loaded with new delay
                            wb2_update <= 1;
                       end
                       else begin
                            o_wb2_data <= odelay_data_cntvaluein[wb2_read_lane];//send ODELAYE2 cntvaluein of the given lane of DQS
                       end
                    ...
                    1: if(wb2_we) begin 
                            ...
                            wb2_phy_odelay_dqs_ld <= wb2_data[5 +: $clog2(LANES)]; //save lane to be loaded with new delay
                            wb2_update <= 1;
                       end
                       else begin
                            o_wb2_data <= odelay_dqs_cntvaluein[wb2_read_lane];//send ODELAYE2 cntvaluein of the given lane of DQS
                       end
                    ...
                    2: if(wb2_we) begin 
                            ...
                            wb2_phy_idelay_data_ld <= wb2_data[5 +: $clog2(LANES)]; //save lane to be loaded with new delay
                            wb2_update <= 1;
                       end
                       else begin
                            o_wb2_data <= idelay_data_cntvaluein[wb2_read_lane];//send ODELAYE2 cntvaluein of the given lane of DQS
                       end
                    ...
                    3: if(wb2_we) begin 
                            ...
                            wb2_phy_idelay_dqs_ld <= wb2_data[5 +: $clog2(LANES)]; //save lane to be loaded with new delay
                            wb2_update <= 1;
                       end
                       else begin
                            o_wb2_data <= idelay_dqs_cntvaluein[wb2_read_lane];//send ODELAYE2 cntvaluein of the given lane of DQS
                       end

                    //read/write the lane number to be read later
                    4: if(wb2_we) begin //write the lane number to be read later
                            wb2_read_lane <= wb2_data[$clog2(LANES)-1:0]; 
                       end
                       else begin
                           o_wb2_data <= wb2_read_lane;
                       end

              default: if(!wb2_we) begin 
                           o_wb2_data <= {WB2_DATA_BITS{1'b1}}; //return all 1s when address to be read is invalid 
                       end
                endcase
