...
module ddr3_controller #(
    ...
    parameter // The next parameters act more like a localparam (since user does not have to set this manually) but was added here to simplify port declaration
                ...
                wb_addr_bits = ROW_BITS + COL_BITS + BA_BITS - $clog2(wb_data_bits/ 8),
    ...
    function [27:0] read_rom_instruction(input[4:0] instruction_address); begin
    ...
    always @(posedge i_controller_clk, negedge i_rst_n) begin
        ...
        // can only start accepting requests  when reset is done
            ...
            if(i_wb_cyc && !o_wb_stall) begin 
                ...
                stage1_col <= { i_wb_addr[(COL_BITS- $clog2(wb_data_bits/8)-1):0], {{$clog2(wb_data_bits/8)}{1'b0}} }; //column address (n-burst word-aligned)
                stage1_bank <=  i_wb_addr[(BA_BITS + COL_BITS- $clog2(wb_data_bits/8) - 1) : (COL_BITS- $clog2(wb_data_bits/8))]; //bank_address
                stage1_row <= i_wb_addr[ (ROW_BITS + BA_BITS + COL_BITS- $clog2(wb_data_bits/8) - 1) : (BA_BITS + COL_BITS- $clog2(wb_data_bits/8)) ]; //row_address
                ...
                {stage1_next_row , stage1_next_bank} <= wb_addr_plus_anticipate[(ROW_BITS + BA_BITS + COL_BITS- $clog2(wb_data_bits/8) - 1) : (COL_BITS- $clog2(wb_data_bits/8))];
...
`endif
        ...
        integer result;                              
        begin
            result = $rtoi($ceil(ns*1.0/CONTROLLER_CLK_PERIOD)); 
            ...
            if(i_wb_cyc) begin //only if already done calibrate and controller can accept wb request 
                ...
                if(cmd_d[WRITE_SLOT][CMD_CS_N:CMD_WE_N] == 4'b0100) begin //WRITE
                    if(state_calibrate == DONE_CALIBRATE) begin
                       ...
                       f_read_data_col = {f_read_data[1 +: COL_BITS - $clog2(wb_data_bits/8)], {$clog2(wb_data_bits/8){1'b0}} }; //column address must match 
                       assert(cmd_d[WRITE_SLOT][CMD_ADDRESS_START:0] == f_read_data_col);

                       f_read_data_bank = f_read_data[(COL_BITS - $clog2(wb_data_bits/8)) + 1 +: BA_BITS]; //bank must match 
        ...
        always @* begin
            if(!f_empty && !f_full) begin //make assertion when there is only 1 data on the pipe
                if(stage1_pending) begin //request is still on stage1
                    assert(stage1_bank == f_read_data[(COL_BITS - $clog2(wb_data_bits/8)) + 1 +: BA_BITS]); //bank must match 
                    assert(stage1_col == {f_read_data[1 +: COL_BITS - $clog2(wb_data_bits/8)], {$clog2(wb_data_bits/8){1'b0}}}); //column address must match 
                    assert(stage1_we == f_read_data[0]); //i_wb_we must be high
                end
                if(stage2_pending) begin //request is now on stage2
                    assert(stage2_bank == f_read_data[(COL_BITS - $clog2(wb_data_bits/8)) + 1 +: BA_BITS]); //bank must match 
                    assert(stage2_col == {f_read_data[1 +: COL_BITS - $clog2(wb_data_bits/8)], {$clog2(wb_data_bits/8){1'b0}}}); //column address must match 
            ...
            if(f_full) begin //both stages have request
                ...
                //handles by stage1)
                assert(stage1_bank == f_read_data_next[(COL_BITS - $clog2(wb_data_bits/8)) + 1 +: BA_BITS]); //bank must match 
                assert(stage1_col == {f_read_data_next[1 +: COL_BITS - $clog2(wb_data_bits/8)], {$clog2(wb_data_bits/8){1'b0}}}); //column address must match 
